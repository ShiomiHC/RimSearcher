...
起因是一句范围质疑:「是否真的有必要这么多功能,一开始只想看看区别,现在是在拓展什么吗」。
先量再答 —— 量的结果把「拓展」这个前提反过来了:本仓 19970 行(含 6210 行测试)干着旧世系
master 70900 行的活,上游 3.x 是 1288 行但答不了同一批问题。**规模不是问题,所以收束的目标
不是行数,是「要学几条命令」和「答错之后几步能回来」。**

### 判据(按能推翻别的判据排序)

1. **先画依赖图,再看调用数。** 一条命令的可达性等于它所有消费方的可达性之和;只喂给别的
   命令的东西,继承那条命令的可达性。判「死」之前先问:调用方是谁,那个调用方在这次测量里
   跑得起来吗。**这条要一直传到根,不能只挡一层** —— 本轮就是在这儿栽的(见下)。
2. **零调用 ≠ 冗余,先分清是哪一种零。** 三种成因要的结论完全不同:测量工具够不着的
   (盲测禁写:`export` / `datamod` / `snapshot import` / `sources sync` 及其上游)、
   没人被指过去的(路由表里没有它 —— 那是 skill 的洞不是命令的洞)、真没人要的。
3. **看输出里有没有句子指向它。** 每一处 `'rimsearcher <cmd>'` 都是别的命令落空时指的路。
   删了命令不改这些句子,就是「指路指了个空」。**引用网就是改动清单**,而冷启动死路里的
   引用权重最高,不是最低。
4. **调用有效率,不只是调用数。** 本轮唯一全空跑的是 `fields`(2 次 2 空);而
   `snapshot truncated` / `sources list` / `mods` 各只有两三次却次次有效 —— 五次险些答错里
   有四次是被它们的输出拧回来的。低频是因为被指路才调,不是因为没用。
5. **代码量除以证据量。** 单独看行数会把大而正确的东西排到前面。
6. **问的是命令太多,还是参数一词多义。** 第八轮的失误集中在后者(`--path` 三种含义、
   `code_default` 四态、`--limit 0` 反着来),那类问题删命令一条都解决不了。
7. **最后过一遍 skill 判据**:这条删减改变了调用方该怎么用它吗。改变了才进 SKILL.md。
   收束方法论本身**不进 SKILL** —— 它管的是怎么裁这个工具,不管怎么调它。

### 落地

- `types` 并进 `list`(`3334b42`):同一张表的两个层级,判据是给没给 def 类型。顶层命令
  27 → 26。**不是净省代码**(+104/-65)—— 省的是命令面和一跳:落空那句「没有这个 def 类型」
  现在指得回同一条命令。`types` / `def-types` 留作别名(命令级别名从来不进 `--help` 与
  cli-reference,于是文档面收成一条而老拼法仍然落得下)。
- `modlist` **出局,并补上它的第一道闸**(`34ac880`)。它一度被排成头号删除候选,依据是
  「0 次调用 + 1 处弱内部引用」,两条都错:`export --modlist` 是 `Required = true`,
  没有 mod 列表就产不出快照;那处「弱」引用在两条冷启动死路里。**判断方法本身也错了** ——
  已经给 `export` 挂了「盲测禁写不算证据」的免责,却转头拿调用数判它的必填输入,
  零调用没有沿依赖链传上去。这就是判据 1 的由来。
- 合并在 `d53196a`,两处冲突加**一处 git 自动合掉的语义塌陷**(两侧各自删掉
  `Promises("defs")`,理由相反,合出来那个键无人认领)—— 细节在提交信息里。

### 结论

**停在 `types` 这一刀。** 这套命令面基本没有死肉:`modlist` 是自动化入口,信任层(snapshot /
sources / mods)低频但每次都在拧回答案,`keyed` 是前几轮盲测真实落空的产物。剩下 `fields`
在观察名单上(2 次全空跑,但有 3 处落空指路),而真要动是把它与 `values` 合成同一张表的转置
—— 那是设计变更不是删减,不在收束范围。

顺带查出来两处「最该有闸的地方没有闸」,都已补:`modlist` 407 行是 `export` 的必填输入却
一道闸都没有;`help-overview.txt` 这份基线装的是一句 `Unknown command '--help'` 而不是总览
(成因是 `Runner` 要求 `argv.Count == 1`,而 fixture 恒追加 `--db` / `--config`),
于是顶层帮助从来没被闸住过 —— 修在 `9ae2463`。
