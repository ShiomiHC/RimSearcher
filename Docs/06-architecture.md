...
4. **快照**:多快照,身份 = db meta 里的 modlist 指纹;`ModsConfig.xml` 比对自动检测;
   config.toml 只放路径与别名,不复制指纹。
...
- **快照选择** = 用哪次导出:默认走自动检测(读 `ModsConfig.xml` ↔ 各快照 meta 指纹
  比对;命中即选中,指纹同而版本漂 = 02-4 过期警告,无命中 = 明说请进游戏导出),
  `--db <path>` 显式覆盖。过期自证与快照寻址是同一次比对的两个产出。
...
- `snapshot use` 是否需要(有自动检测后,手动固定可能多余):先不做,盲测看需求。
