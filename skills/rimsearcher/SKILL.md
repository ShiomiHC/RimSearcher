...
| Does the vanilla XML write this line — Replace or Add? | `rimsearcher get <defName> --defaults` — the `xml` column (`here` / `parent` / `no` / `under <container>`). Only on snapshots from exporter 0.5.0; older ones omit the column and say so. `here`/`parent` include list entries named after a def (`costList.Steel`) once a sibling value on that index entry names the tag. From exporter 0.6.0 the snapshot also records that tag's inline text, so a short form `<Steel>75</Steel>` can pin `count` as `here` and the unwritten sibling as `no`. From exporter 0.7.0 the paths are read from the merged XML **after every PatchOperation ran**, and a line another mod's patch put there reads `here+patch` / `parent+patch` — Replace still finds it, but your patch now depends on that mod staying loaded. `no` is determined, not a path-shape maybe. On older snapshots the column reads the XML on disk before any patch ran, so a patched-in node reads `no` there instead; `rimsearcher inherit <defName>` counts the patch xpaths that name the def. `under <container>` is the leftover: the entry did not join, or the inline text matches none or more than one remaining field, or this snapshot predates recording that text — neither answer is available for that row. |
