...
| `--lines` <a-b|a+n|a|all> | Read raw lines instead: '400-460' is inclusive, '400+60' is sixty lines from 400, '400' starts there and takes 150 lines, 'all' is the whole file however long it is. Whatever it asks for is printed in full unless --limit says otherwise. Without it the whole file is read. | `--line`, `--range`, `--line-range` |
