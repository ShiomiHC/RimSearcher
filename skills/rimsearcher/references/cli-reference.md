...
| `--path-contains` <text> | Only list paths containing this text. Repeat it to widen the selection. '[]' stands for any index, so a path shape this tool printed — 'comps[].props.energyMax' — goes straight back in. | `--filter`, `--grep`, `--field-contains`, `--path-filter`, `--contains`, `--match` |
...
| `--path-contains` <text> | Only show field paths containing this text. Repeat it to widen the selection. '[]' stands for any index, so a path shape this tool printed — 'comps[].props.energyMax' — goes straight back in. | `--filter`, `--grep`, `--field-contains`, `--path-filter`, `--field`, `--field-path`, `--path` |
