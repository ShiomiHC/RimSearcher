...
When present, the 'xml' column says whether this def's own XML wrote the path (here), only an ancestor did (parent), or neither (no) — the fact PatchOperationReplace vs Add turns on. A fourth value, 'under <container>', means the XML wrote that container but names its entries after defs (costList.Steel) where the index numbers them (costList[0].thingDef), so this row joins to nothing and neither answer is available. Older snapshots omit the column and say so.
