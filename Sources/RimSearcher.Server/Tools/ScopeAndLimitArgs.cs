...
public readonly record struct ResultLimit(int Count, bool Unlimited)
{
    ...
    public ResultLimit Scale(int factor)
    {
        ...
        return new ResultLimit((int)Math.Clamp(scaled, 1, ScopeAndLimitArgs.HardLimit), Unlimited);
    }
}
...
public static class ScopeAndLimitArgs
{
    ...
    private static string DescribeLimitValue(JsonElement value) => value.ValueKind switch
    {
        ...
        JsonValueKind.Array => $"an array of {CountedNoun.Items.Quantity(value.GetArrayLength())}",
