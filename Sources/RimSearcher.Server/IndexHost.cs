...
public static class IndexHost
{
    ...
    private static string BuildMutexName(string pipeName) => $@"Global\{pipeName}.mutex";
    ...
    public static async Task<bool> TryRunAsProxyAsync(string configFingerprint, TextWriter protocolOut)
    {
        ...
        for (var attempt = 0; attempt < 3; attempt++)
        {
            ...
            if (!MutexHeldByAnotherProcess(pipeName)) return false;
    ...
    private static bool MutexHeldByAnotherProcess(string pipeName)
    {
        try
        {
            using var mutex = new Mutex(false, BuildMutexName(pipeName), out _);
            if (mutex.WaitOne(0))
            {
                mutex.ReleaseMutex();
                return false;
    ...
    // 宿主侧：抢锁成功才建索引并对外服务。返回持有的 mutex（须存活到进程结束）。
    public static Mutex? TryBecomeHost(string configFingerprint)
    {
        ...
        try
        {
            var mutex = new Mutex(false, BuildMutexName(pipeName), out _);
            if (!mutex.WaitOne(0))
            {
                mutex.Dispose();
                return null;
        ...
        catch (AbandonedMutexException)
        {
            // 上一任宿主异常退出，锁归本进程
            _isHost = true;
            return new Mutex(true, BuildMutexName(pipeName), out _);
        }
        catch (Exception)
        {
            return null;
