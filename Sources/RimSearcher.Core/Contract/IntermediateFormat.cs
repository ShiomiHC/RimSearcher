...
namespace RimSearcher.Contract
{
    public static class IntermediateFormat
    {
        ...
        //
        // **一个槽位一行,一个不漏**(0.9.0 起)。这一层是**名册**而不是补充说明:导入侧要问的
        // 是「译者写的这个键在不在册」,而那一问只有全集答得出。0.8.0 那版只发「带信息」的行
        // (两键不同的、或不许译且有文本的),省下的是「绝大多数只把字段表又说一遍」的行 ——
        // 而省掉之后问不出口,判据退化成拿字段表当替身:整表注入的键不带元素下标
        // (rulePack.rulesStrings),字段表里那条路径带(…rulesStrings[0]),逐字比一次不中。
        // 实测 1348 条判「配不上槽位」里 956 条是这么冤枉的,还配着一句「游戏那边同样注入
        // 不上」的假话。省行数省出一句假话,是这一层唯一不该做的交换。
        //
