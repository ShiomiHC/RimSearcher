...
///   2. **变更判据** —— 来源 dll 的哈希。对得上就不必重跑(vanilla 一万多个文件,
///      白跑一次是几十秒加一整屏假 diff)。
///   3. **产地记录** —— 它跟着源码一起进 git,于是历史里每一次源码变化旁边都写着
///      「是哪几个 dll 的哪个版本产生的」。这是自制 diff 从来给不了的东西。
///
/// 刻意**不记时间戳**:那是 git 提交时间的活儿,写进来就等于给同一件事造第二个产地,
/// 还会让每次同步都无端改一行。
/// </summary>
