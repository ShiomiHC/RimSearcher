...
public sealed class Report
{
    ...
    /// 「pass --offset N for the next page」,而 `--offset` **全史使用 0 次**(07/13 的
    /// 逐命令普查,别名也全是 0)。留用理由曾是「N 是算出来的,省一次 off-by-one」,
    /// 而它省的是一次查表,不是防一次误判。**这半条依据是使用率,仍然成立。**
    ///
    /// (2026-08-05 更正:此处原写「印了 34 次而使用 0 次」—— 34 是 08 那张字节预算表的
    /// **字符数**,不是印出次数,那半句从没被量过。判掉它靠的一直是「零使用」这一个数。)
    ///
