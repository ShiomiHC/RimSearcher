...
public sealed class Report
{
    ...
    /// 「pass --offset N for the next page」,而 `--offset` 全史 **3 次 / 6779 次调用**
    ...
    /// ④ 3 次 / 6833 —— 按仓名排还是不够,消费方自己目录下还跑着 16 份闭卷题
    /// (62 次调用),得按**题面**认。
    /// **一句结论连着四个论据都是错数而结论没塌**,这件事本身值得记:它说明当时判掉它靠的
    /// 其实是别的东西,而写下来的数是事后配的。同时注意方向不一致 —— ② 让真调用读成 0,
    /// ③④ 让 0 读成非零,坏的都是「有没有人用」这个判据本身。)
    ///
    ...
    ///
    /// (此前这里拿 <c>read</c> 截断行上点名 <c>--outline</c> 的那句当正面对照 ——「八个盲测被试
    /// 全部用上了」。那句 2026-09-20 按真实语料删了:2090 次实印后只有 3% 去 outline,且不随
    /// 页数变。盲测那 8/8 两臂都带着它,量的不是它。对照不成立,但本条的结论不靠它。)
    ///
