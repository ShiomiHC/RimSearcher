...
public sealed class Report
{
    ...
    /// 「pass --offset N for the next page」,而 `--offset` 全史 **4 次 / 8180 次调用**
    /// (13 的逐命令普查)。留用理由曾是「N 是算出来的,省一次 off-by-one」,而它省的是
    /// 一次查表,不是防一次误判。**这半条依据是使用率,仍然成立。**
    ///
    /// (2026-08-05 两次更正,同一处:先是「印了 34 次」—— 34 其实是 08 字节预算表的
    /// **字符数**,不是次数;继而「使用 0 次」也不对 —— 普查脚本只扫会话目录顶层,
    /// subagent 发起的七成流量从没进过统计。**一句结论连续两个论据都是错数而结论没塌**,
    /// 这件事本身值得记:它说明当时判掉它靠的其实是别的东西,而写下来的两个数是事后配的。)
    ///
