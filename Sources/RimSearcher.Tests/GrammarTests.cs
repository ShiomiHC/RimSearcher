...
public class GrammarTests
{
    ...
    /// 新成因,分不清就会报最强的那种。
    ///
    /// **哪一页都不给下一页的参数。** `--offset` 全史使用 0 次(07/13 逐命令普查,别名
    /// 也全是 0),而那句提示省的是一次查表,不是防一次误判 —— 它占的却是 line 1,
    /// 管道下唯一的幸存者。这半条依据是使用率实证,不受下面那条更正影响。
    /// (此处原写「印了 34 次而使用 0 次」:34 是 08 字节预算表的**字符数**,不是次数。)
    ///
    ...
    ///
    /// 带 offset 那格**没被盲测覆盖**(本轮零样本用 `--offset`,而它全史使用 0 次),
