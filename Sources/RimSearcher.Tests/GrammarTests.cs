...
public class GrammarTests
{
    ...
    /// 新成因,分不清就会报最强的那种。
    ///
    /// **哪一页都不给下一页的参数。** `--offset` 全史 4 次 / 8180 次调用(13 逐命令普查),
    /// 而那句提示省的是一次查表,不是防一次误判 —— 它占的却是 line 1,管道下唯一的幸存者。
    /// (这个数订正过两次:先前写「印了 34 次」,34 是 08 字节预算表的**字符数**;继而写
    /// 「使用 0 次」,而普查漏了 subagent 那七成流量。结论没变,产地见 <c>Report.PageNotice</c>。)
    ///
    ...
    ///
    /// 带 offset 那格**没被盲测覆盖**(本轮零样本用 `--offset`,而它全史只有 4 次 / 8180),
