...
public class GrammarTests
{
    ...
    /// 新成因,分不清就会报最强的那种。
    ///
    /// **哪一页都不给下一页的参数。** `--offset` 全史 3 次 / 6779 次调用(13 逐命令普查),
    /// 而那句提示省的是一次查表,不是防一次误判 —— 它占的却是 line 1,管道下唯一的幸存者。
    /// (这个数订正过三次:「印了 34 次」是把字节预算表的字符数当成次数;「使用 0 次」是
    /// 普查漏了 subagent 那七成流量;「4 次 / 8180」是分母里混着盲测仓 rsblind 的 1353 次。
    /// 结论三次都没变,产地见 <c>Report.PageNotice</c>。)
    ///
    /// **原先同处还写着一句「`2 of 9 defs` 自己就带着截断信号」—— 那句已被实测推翻。**
    /// 它当时同时写在实现注释(<c>Report.PageNotice</c>)和这条闸的注释里,两份不是独立
    /// 判断而是同一份判断的副本,所以从内部读只看到自洽,绿了很久。
    /// 30 个 haiku 闭卷样本:旧形态在裸截断行上 0/7,现在这个形态 7/7
    /// (口径与代价见 <see cref="Tally.RenderTotalFirst"/>)。
    ///
    /// **无 offset 那格的断言不许放宽成子串**:`9 defs, showing the first 2.` 里两个数
    /// 缺一个都不算 —— 而恰恰是「只印一个数」这种退化最容易在重构里悄悄发生。
    ///
    /// 带 offset 那格**没被盲测覆盖**(本轮零样本用 `--offset`,而它全史只有 3 次 / 6779),
    /// 所以那句话是按同一条推理推出来的,不是测出来的。别把 7/7 记到它头上。
    /// </summary>
    [Fact]
    ...
    public void 子串匹配要说破自己不是整段命中()
    {
        ...
        //  两种读法让人挑,机制自己已经说全,已删。)
        Assert.Contains("Nothing is filtered out for that", get0, StringComparison.Ordinal);
        Assert.Contains("every match is in the table below", get0, StringComparison.Ordinal);
    ...
    public void 一致这句话要同时说清没比的是什么()
    {
        ...
        Assert.Matches(@"xml_fingerprint +not recorded \(exported before this was measured\)", stdout);
