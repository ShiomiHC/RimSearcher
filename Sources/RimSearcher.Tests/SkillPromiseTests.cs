...
public class SkillPromiseTests
{
    ...
    [
        ...
        new("`snapshot diff <old> <new>` compares two snapshots' resolved defs and fields",
            nameof(SnapshotDiffTests.同名单一字段变了加了一个删了一个)),
        new("does not take `--scope`",
            nameof(SnapshotDiffTests.声明与help都不提供scope)),
        new("Snapshots exported with different ordered mod lists are refused",
            nameof(SnapshotDiffTests.名单差拒比并指路mod_list)),
        new("A comparison that has any difference prints `0 defs added.`",
            nameof(SnapshotDiffTests.文本零的一侧仍报零而不是整张表消失)),
        new("pass `--name <other>` to keep both",
            nameof(SnapshotRetentionTests.第三次同名拒并指路改名)),
        new("A re-export whose resolved defs and fields already match leaves both files alone",
            nameof(SnapshotRetentionTests.相同内容跳过且不挤掉prev)),
