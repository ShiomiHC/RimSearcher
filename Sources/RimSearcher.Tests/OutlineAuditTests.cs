...
///
/// 来历:一次全树自检(19467 个反编译文件 / 212922 条声明)撞出两类错,而它们**都不是**
/// 「少认一个成员」那种可以先欠着的缺陷 —— 两类的产物都是**带着正确行号的错名字**,
/// 也就是这套输出契约列为头号禁忌的那个形状:错答案穿着对答案的衣服,而
/// <c>SayBraceMatched</c> 那句诚实的边界声明反过来成了它的掩护,读的人会去改拼写。
///
///   · 元组类型(vanilla 23 个文件)。声明头里第一个顶层 '(' 是**类型**不是参数表,
///     取它左边的标识符就取到修饰符。`CellRect.SplitVertical` 曾整个消失,
///     列里留下一个叫 public 的方法,行号 796-804 分毫不差。
///   · 泛型约束连写(8 个文件)。`where T : class where U : struct` 里的 `class where`
///     是「关键字 + 空格 + 标识符」,被认成类型声明后以类型身份压栈,
///     <c>Declarable</c> 跟着放行 —— **整个方法体的语句都变成声明**,一处误判毁一整块。
///
/// 这里不扫真实树:那棵树在哪、装了哪些 mod 因机器而异,闸不能建在这种地方。
/// 语料改为把撞出来的形态逐条内联,判据沿用自检那套 —— 关键字不许出现在名字位置。
/// 判据是**反向**的(不许有什么),所以每条语料同时配一份正向名单(必须认出什么):
/// 只判反向的话,一个把所有声明都丢掉的实现同样能过。
/// </summary>
public class OutlineAuditTests
{
    ...
    ///
    /// 期望值写成全集而不是「至少包含」:多认一个语句碎片与少认一个成员,在这份轮廓里
    /// 是同等严重的两件事,而「至少包含」对前者一个字都说不出来。
    /// </summary>
    ...
    /// <summary>
    /// 全树自检的判据本体:关键字不许落在名字位置。
    ///
    /// 一条语句被当成声明,产出的名字必然是个关键字(if / while / return);一个声明头被
    /// 切错,产出的名字必然是个修饰符(public / private / static)。两类都被这一条罩住,
    /// 而它对**没见过的**新形态同样有效 —— 上面那张表只认得已经撞过的那些。
    /// </summary>
    [Theory]
    [MemberData(nameof(Shapes))]
