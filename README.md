...
- CamelCase 缩写与拼写容错（如 `JDW`）。**容错对成员段最弱**：成员模糊匹配的候选池按索引枚举序硬截 200 条，本机这种量级的索引下真值几乎必然落选，于是 `method:CompTickRar` 在有 `CompTickRare` 的语料里一条也不回。**空的 Members 段不是「这个成员不存在」的证据**——换准确拼写重查，或改用 `search_regex`（工具描述里也这么写）
