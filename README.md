...
- CamelCase 缩写与拼写容错（如 `JDW`）。**成员段与其余三段同等**：`method:CompTickRar` 会给出 `CompTickRare`。成员模糊匹配的候选池曾按索引枚举序硬截 200 条——真实规模的索引下光查询的头一个 2-gram 就能填满配额，真值几乎必然落选——现已改为按 2-gram 重合度给全部成员键排名后取前 500，选出来的那一批与索引枚举序无关
