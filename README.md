...
- 这两个工具还共用一句 `... some files were not scanned in full (…)`：有文件读不开、或大到只扫了前 20000 行时才出现。**它一出现，表头的命中数就改口成 `at least N`**（那时那个数只是下界）——反过来说，表头写着 `N found` 就是确定值
...
- `usages`：符号的**逐行文本匹配**（不分大小写的全词匹配），C# 与 XML 都扫，带行号预览。默认 50 条，每个文件最多 3 行预览，其余记为 `+N more in this file`
  ...
  - 同款的还有完整性契约：读不开的文件与只扫到 20000 行的大文件会在末尾计数上报，同时表头改口 `at least N`。没有那句尾注、表头写着 `N found`，这 N 就是该 scope 下的确定总数
