...
- 结果行不重复印能从符号名逐字推出来的文件名（见 `locate` 一节）——留下的每一个文件名都是「它不在同名文件里」的信号
...
- **同一条查询恒给同一份答案**。扫描按候选表顺序分块推进、命中按 `(文件序号, 行号)` 排序后再截，所以截断时拿到的是候选表的**前缀**：复查一遍是同一批文件，把 `limit` 调大只会往后接上更多，不会把先前给过的换掉。展示时再按文件名排序
