...
- CamelCase 缩写与拼写容错（如 `JDW`）。**成员段与其余三段同等**：`method:CompTickRar` 会给出 `CompTickRare`。成员模糊匹配的候选池曾按索引枚举序硬截 200 条——真实规模的索引下光查询的头一个 2-gram 就能填满配额，真值几乎必然落选——现已改为按 2-gram 重合度给全部成员键排名后取前 500，选出来的那一批与索引枚举序无关。这 500 条是这条链上仅剩的一道限额，它一旦装不下同等好的匹配，表头会改口 `at least`（见「结果分段」）——限额本身仍在，但不再是一道看不见的限额
...
**结果分段**：`C# Types` / `Members` / `XML Defs` / `Content Matches`（按 Def 的字段值命中，而非按名），每段各自受 `limit` 约束并独立折叠。表头一行给出**各段列出了几条、这个 scope 里一共有几条**（`## 'Pawn' — 1 of 768 C# types, 3 of 1931 members`），不必自己数行、也不必拿折叠行去做加法就能判断要不要调 `limit`。没被截断的段不写 `of N`——**看到 `of` 就是被截了**，与折叠行是同一条读法。`Members` 段的总数还有一种形态 `N of at least M`：成员模糊匹配的候选池（500 个成员键）被**同等好的**名字匹配填满时，M 只是地板而不是总数（`method:Notify_` 这类共同前缀成百上千的查询会撞到），要确数得收窄查询。判据是「进池的键是否**全部**够分」而不是「池子满没满」——后者在真实规模的索引上几乎恒真，会退化成常亮，故 `method:CompTickRar` 那种池里只有一个够分的仍给确数。与扫描类工具的 `at least N matching lines` 是同一条读法：**出现 `at least` 就说明这个数只是下界**。（这里与 `trace inheritors` 的 `(381 …) Listed below: 200` 是同一个口径，只是排版不同：两个工具的表头都同时给总数与显示数。）另有 `Files` 段（已索引的文件路径）：四段全部零命中时它是兜底，按名模糊列出若干条；四段有命中时它只补上**基名与查询词逐字相同**的那一份，且不重复已经出现在 `C# Types` 里的同名项——文件名是一等查询目标，不该因为顺带蹭到一条低分 def 就整段消失。
